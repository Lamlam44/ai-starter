...
MODEL_DIR = 'src/DecisionTree/saved_models'
MODEL_FILENAME = 'decision_tree_model.pkl'
LABEL_ENCODERS_FILENAME = 'label_encoders_dt.pkl'
TARGET_ENCODER_FILENAME = 'target_encoder_dt.pkl'
FEATURE_COLUMNS_ORDER_FILENAME = 'feature_cols_order_dt.pkl'
CATEGORICAL_FEATURE_COLUMNS_USED_FILENAME = 'categorical_features_used_dt.pkl'
FILLNA_VALUE_FILENAME = 'fillna_value_dt.pkl'
...
try:
    ...
    print(f"Đã tải mô hình Decision Tree và các đối tượng tiền xử lý thành công. Giá trị fillna: {fillna_value}")
except FileNotFoundError as e:
    messagebox.showerror("Lỗi Khởi động", f"Không tìm thấy file .pkl: {e.filename}.\nĐảm bảo thư mục 'saved_models' trong 'src/DecisionTree' tồn tại và chứa các file này.")
    sys.exit()
...
try:
    ...
    # Điền các giá trị NA bằng 'unknown' trước khi lấy các giá trị duy nhất
    for col in ['Sex', 'Housing', 'Saving accounts', 'Purpose', 'Checking account']: # Thêm 'Checking account' vào đây
...
# --- Tạo giao diện ---
root = tk.Tk()
root.title("Dự đoán Rủi ro Tín dụng (Decision Tree)")
...
def on_train_button_click():
    from train_decision_tree import train_and_evaluate_decision_tree

    messagebox.showinfo("Bắt đầu Huấn luyện", "Quá trình huấn luyện mô hình Decision Tree đang được tiến hành...")
    evaluation_metrics = train_and_evaluate_decision_tree(DATA_PATH)
    if evaluation_metrics:
        ...
        evaluation_window.title("Kết quả Đánh giá Mô hình Decision Tree")
        ...
        global model, label_encoders, target_le, FEATURE_COLUMNS_ORDER, CATEGORICAL_FEATURE_COLUMNS_USED, fillna_value
        try:
            model = joblib.load(MODEL_PATH)
            label_encoders = joblib.load(LABEL_ENCODERS_PATH)
            target_le = joblib.load(TARGET_ENCODER_PATH)
            ...
            CATEGORICAL_FEATURE_COLUMNS_USED = joblib.load(CATEGORICAL_FEATURE_COLUMNS_USED_PATH)
            fillna_value = joblib.load(FILLNA_VALUE_PATH)
            print(f"Đã tải lại mô hình Decision Tree và các đối tượng tiền xử lý sau khi huấn luyện. Giá trị fillna: {fillna_value}")
        except FileNotFoundError as e:
        ...
        messagebox.showerror("Lỗi Huấn luyện", "Đã xảy ra lỗi trong quá trình huấn luyện mô hình Decision Tree.")
