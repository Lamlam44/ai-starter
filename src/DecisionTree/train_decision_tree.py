...
def train_and_evaluate_decision_tree(DATA_PATH):
    """
    Huấn luyện và đánh giá mô hình Decision Tree, trả về các chỉ số đánh giá.
    ...
    try:
        ...
        CATEGORICAL_FEATURE_COLUMNS_USED = [col for col in CATEGORICAL_FEATURE_COLUMNS if col in df.columns and col in FEATURE_COLUMNS]
        for col in list(CATEGORICAL_FEATURE_COLUMNS):
            if col not in CATEGORICAL_FEATURE_COLUMNS_USED:
                print(f"\nCảnh báo: Không tìm thấy cột phân loại '{col}' hoặc không nằm trong danh sách thuộc tính.")
        ...
        OUTPUT_DIR = 'src/DecisionTree/saved_models'
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        joblib.dump(model, os.path.join(OUTPUT_DIR, 'decision_tree_model.pkl'))
        joblib.dump(label_encoders, os.path.join(OUTPUT_DIR, 'label_encoders_dt.pkl'))
        joblib.dump(target_le, os.path.join(OUTPUT_DIR, 'target_encoder_dt.pkl'))
        joblib.dump(FEATURE_COLUMNS, os.path.join(OUTPUT_DIR, 'feature_cols_order_dt.pkl'))
        joblib.dump(CATEGORICAL_FEATURE_COLUMNS_USED, os.path.join(OUTPUT_DIR, 'categorical_features_used_dt.pkl'))
        joblib.dump('unknown', os.path.join(OUTPUT_DIR, 'fillna_value_dt.pkl')) # Lưu 'unknown' làm giá trị fillna

        LOG_DIR = 'logs'
        os.makedirs(LOG_DIR, exist_ok=True)
        LOG_FILENAME = os.path.join(LOG_DIR, 'training_log_dt.txt')

        with open(LOG_FILENAME, 'w', encoding='utf-8') as f:
            f.write("--- Báo cáo Huấn luyện Mô hình Decision Tree ---\n\n")
