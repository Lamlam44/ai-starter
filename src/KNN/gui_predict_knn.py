...
MODEL_DIR = 'src/KNN/saved_models'
MODEL_FILENAME = 'knn_model.pkl'
LABEL_ENCODERS_FILENAME = 'label_encoders_knn.pkl'
TARGET_ENCODER_FILENAME = 'target_encoder_knn.pkl'
SCALER_FILENAME = 'scaler_knn.pkl'
FEATURE_COLUMNS_ORDER_FILENAME = 'feature_cols_order_knn.pkl'
CATEGORICAL_FEATURE_COLUMNS_FILENAME = 'categorical_features_knn.pkl'
NUMERICAL_FEATURE_COLUMNS_FILENAME = 'numerical_features_knn.pkl'
FILLNA_VALUE_FILENAME = 'fillna_value_knn.pkl'
...
try:
    ...
    print(f"Đã tải mô hình KNN và các đối tượng tiền xử lý thành công. Giá trị fillna: {fillna_value}")
except FileNotFoundError as e:
    messagebox.showerror("Lỗi Khởi động", f"Không tìm thấy file .pkl: {e.filename}.\nĐảm bảo thư mục 'saved_models' trong 'src/KNN' tồn tại và chứa các file này.")
    sys.exit()
...
# --- Tạo giao diện ---
root = tk.Tk()
root.title("Dự đoán Rủi ro Tín dụng (KNN)")
...
def on_train_button_click():
    from train_knn import train_and_evaluate_knn

    messagebox.showinfo("Bắt đầu Huấn luyện", "Quá trình huấn luyện mô hình KNN đang được tiến hành...")
    evaluation_metrics = train_and_evaluate_knn(DATA_PATH)
    if evaluation_metrics:
        ...
        evaluation_window.title("Kết quả Đánh giá Mô hình KNN")
        ...
        try:
            ...
            NUMERICAL_FEATURE_COLUMNS = joblib.load(NUMERICAL_FEATURE_COLUMNS_PATH)
            fillna_value = joblib.load(FILLNA_VALUE_PATH)
            print(f"Đã tải lại mô hình KNN và các đối tượng tiền xử lý sau khi huấn luyện. Giá trị fillna: {fillna_value}")
        except FileNotFoundError as e:
        ...
        messagebox.showerror("Lỗi Huấn luyện", "Đã xảy ra lỗi trong quá trình huấn luyện mô hình KNN.")
