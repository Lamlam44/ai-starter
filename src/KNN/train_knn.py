...
def train_and_evaluate_knn(DATA_PATH):
    """
    Huấn luyện và đánh giá mô hình K-Nearest Neighbors (KNN), trả về các chỉ số đánh giá.
    ...
    try:
        ...
        model = KNeighborsClassifier(n_neighbors=5) # Bạn có thể thử các giá trị K khác
        ...
        OUTPUT_DIR = 'src/KNN/saved_models'
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        joblib.dump(model, os.path.join(OUTPUT_DIR, 'knn_model.pkl'))
        joblib.dump(label_encoders, os.path.join(OUTPUT_DIR, 'label_encoders_knn.pkl'))
        joblib.dump(target_le, os.path.join(OUTPUT_DIR, 'target_encoder_knn.pkl'))
        joblib.dump(scaler, os.path.join(OUTPUT_DIR, 'scaler_knn.pkl'))
        joblib.dump(FEATURE_COLUMNS, os.path.join(OUTPUT_DIR, 'feature_cols_order_knn.pkl'))
        joblib.dump(CATEGORICAL_FEATURE_COLUMNS, os.path.join(OUTPUT_DIR, 'categorical_features_knn.pkl'))
        joblib.dump(NUMERICAL_FEATURE_COLUMNS, os.path.join(OUTPUT_DIR, 'numerical_features_knn.pkl'))
        joblib.dump('unknown', os.path.join(OUTPUT_DIR, 'fillna_value_knn.pkl')) # Lưu 'unknown' làm giá trị fillna

        LOG_DIR = 'logs'
        os.makedirs(LOG_DIR, exist_ok=True)
        LOG_FILENAME = os.path.join(LOG_DIR, 'training_log_knn.txt')

        with open(LOG_FILENAME, 'w', encoding='utf-8') as f:
            f.write("--- Báo cáo Huấn luyện Mô hình KNN ---\n\n")
